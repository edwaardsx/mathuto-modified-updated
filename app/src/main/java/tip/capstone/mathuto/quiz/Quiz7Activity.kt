...
import tip.capstone.mathuto.databinding.Quiz7Binding
import tip.capstone.mathuto.questions.Question7
...
import tip.capstone.mathuto.quiz.result.Result7Activity
...
class Quiz7Activity : AppCompatActivity(), View.OnClickListener {

    private lateinit var binding: Quiz7Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = Quiz7Binding.inflate(layoutInflater)
        ...
        mMultipleChoiceList = Question7.getQuestions()
    ...
    private fun processOptionSelected() {
        ...
        if (mCurrentPosition == mMultipleChoiceList!!.size || mCurrentPosition >= mMaxQuestions) {
            handler.postDelayed({
                val intent = Intent(applicationContext, Result7Activity::class.java)
                seBackgroundMusic?.stop()
                intent.putExtra(CORRECT_ANS, mCorrectAnswers)

                val scores = db.getAllHighScores()
                if(scores.size < 7){
                    db.insertHighScores("Lesson 7", mCorrectAnswers.toString())
                }else{
                    if (mCorrectAnswers > Integer.parseInt(scores[6].score)) {
                        ...
                        if (mCorrectAnswers > highestScore) {
                            db.updateHighScores("Lesson 7", mCorrectAnswers.toString())
                        }
                    }
                }
