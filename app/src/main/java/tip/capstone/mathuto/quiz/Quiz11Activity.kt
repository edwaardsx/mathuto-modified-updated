...
import tip.capstone.mathuto.databinding.Quiz11Binding
import tip.capstone.mathuto.questions.Question11
import tip.capstone.mathuto.questions.Question11.CORRECT_ANS
import tip.capstone.mathuto.questions.Question11.SELECTED_ANSWERS
import tip.capstone.mathuto.questions.Question11.TOTAL_QUESTIONS
import tip.capstone.mathuto.questions.Question11.WRONG_ANS
import tip.capstone.mathuto.quiz.result.Result11Activity
...
class Quiz11Activity : AppCompatActivity(), View.OnClickListener {

    private lateinit var binding: Quiz11Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = Quiz11Binding.inflate(layoutInflater)
        ...
        mTrueOrFalseList = Question11.getQuestions()
    ...
    private fun processOptionSelected() {
        ...
        if (mCurrentPosition == mTrueOrFalseList!!.size || mCurrentPosition >= mMaxQuestions) {
            handler.postDelayed({
                val intent = Intent(applicationContext, Result11Activity::class.java)
                seBackgroundMusic?.stop()
                intent.putExtra(CORRECT_ANS, mCorrectAnswers)

                val scores = db.getAllHighScores()
                if(scores.size < 11){
                    db.insertHighScores("Lesson 11", mCorrectAnswers.toString())
                }else{
                    if (mCorrectAnswers > Integer.parseInt(scores[10].score)) {
                        ...
                        if (mCorrectAnswers > highestScore) {
                            db.updateHighScores("Lesson 11", mCorrectAnswers.toString())
                        }
                    }
                }
