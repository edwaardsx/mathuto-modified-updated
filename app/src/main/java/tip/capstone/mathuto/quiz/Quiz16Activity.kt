...
import tip.capstone.mathuto.databinding.Quiz16Binding
import tip.capstone.mathuto.questions.Question16
import tip.capstone.mathuto.questions.Question16.CORRECT_ANS
import tip.capstone.mathuto.questions.Question16.SELECTED_ANSWERS
import tip.capstone.mathuto.questions.Question16.TOTAL_QUESTIONS
import tip.capstone.mathuto.questions.Question16.WRONG_ANS
import tip.capstone.mathuto.quiz.result.Result16Activity
...
class Quiz16Activity : AppCompatActivity(), View.OnClickListener {

    private lateinit var binding: Quiz16Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = Quiz16Binding.inflate(layoutInflater)
        ...
        mMultipleChoiceList = Question16.getQuestions()
    ...
    private fun processOptionSelected() {
        ...
        if (mCurrentPosition == mMultipleChoiceList!!.size || mCurrentPosition >= mMaxQuestions) {
            handler.postDelayed({
                val intent = Intent(applicationContext, Result16Activity::class.java)
                seBackgroundMusic?.stop()
                intent.putExtra(CORRECT_ANS, mCorrectAnswers)

                val scores = db.getAllHighScores()
                if(scores.size < 16){
                    db.insertHighScores("Lesson 16", mCorrectAnswers.toString())
                }else{
                    if (mCorrectAnswers > Integer.parseInt(scores[15].score)) {
                        ...
                        if (mCorrectAnswers > highestScore) {
                            db.updateHighScores("Lesson 16", mCorrectAnswers.toString())
                        }
                    }
                }
